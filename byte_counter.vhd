...
--possibly move previous_success into type_pro so it can reset on the 
        begin 
            if rising_edge(clk) then 
                if rst = '1' then 
                    ...
                    if (count_reg + resize(previous_offset, count_reg'length) + 8 >= frame_size_in) and (previous_success = '1') then 
                        count_reg <= (others => '0');
        ...
        byte_count <= count_reg + resize(previous_offset, count_reg'length);
